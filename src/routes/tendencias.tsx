...
const trends = [
  ...
  { t: "AI-First UX", d: "Interfaces conversacionais substituindo formulários tradicionais." },
];
    ...
    <div className="mx-auto max-w-6xl px-6 py-20">
      ...
      <section className="mt-16">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {trends.map((tr) => (
            <div key={tr.t} className="group relative overflow-hidden rounded-2xl border border-border bg-card p-6 transition-colors hover:border-accent/60">
          ...
          {Object.entries(companies).map(([cat, list]) => (
            <div key={cat} className="rounded-2xl border border-border bg-card p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {list.map((co) => (
                  <span key={co} className="rounded-lg border border-border bg-secondary px-3 py-1.5 text-sm">{co}</span>
                ))}
              </div>
            </div>
          ))}
