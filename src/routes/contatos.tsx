  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-20">
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-2">
        {members.map((m) => (
          <article key={m.name} className="group rounded-2xl border border-border bg-card p-6 transition-all hover:-translate-y-1 hover:border-primary/60">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {Object.entries(m.links).map(([label, href]) => (
                <a key={label} href={href} className="rounded-lg border border-border bg-secondary px-3 py-1.5 text-xs font-medium transition-colors hover:bg-primary hover:text-primary-foreground">
                  {label}
                </a>
              ))}
            </div>
      ...
      <section className="mt-16 rounded-3xl border border-border bg-card p-10 text-center">
        ...
        <a href="mailto:contato@exemplo.com" className="mt-6 inline-block rounded-lg bg-[var(--gradient-hero)] px-6 py-3 font-medium text-primary-foreground glow">
          contato@exemplo.com
        </a>
      </section>
    </div>
